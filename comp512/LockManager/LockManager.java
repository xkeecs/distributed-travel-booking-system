	...
	public boolean Lock(int xid, String strData, int lockType) throws DeadlockException, RedundantLockRequestException
	{
		...
		try
		{
			...
			while (bConflict)
			{
				synchronized (this.lockTable)
				{
					...
					if (!bConflict)
					{
						...
						if (bConvert.get(0) == true)
						{
							...
							// then add new objects with WRITE lock
				...
				if (bConflict)
				{
					System.out.println("Waiting other clocks to clear");
					...
					for (int j = 0; j < waitSize; j++)
					{
						...
						if (waitObj.getLockType() == LockManager.WRITE)
						{
							if (j == 0)
							{
								...
								{
									DataObj data = (DataObj) vect1.get(0);
									if (waitObj.getXId() == data.getXId())
									{
										System.out.println("Myself? waitint ????????????");
										if (data.getLockType() == DataObj.READ)
										{
											System.out.println("My Own READ Lock? ????????????");
											this.waitTable.remove(waitObj);
		...
		for (int i = 0; i < size; i++)
		{
			...
			if (dataObj.getXId() == dataObj2.getXId())
			{
				...
				else if (dataObj.getLockType() == DataObj.WRITE)
				{
					...
					// author: kai xiong
					// for case (1), only READ lock is there
					if (dataObj2.getLockType() == DataObj.READ)
					{
						int currentid = dataObj.getXId();
						String currentname = dataObj.getDataName();
						// convert the lock to write if not other Tx holds read, set bitset
						// TrxnObj trxnQueryObj = new TrxnObj(dataObj.getXId(), "", -1); // Only used in elements() call
						// remove other read locks for other Txs first.

						/*
						 * synchronized (this.lockTable) { Vector vectc = this.lockTable.elements(trxnQueryObj);
						 * 
						 * TrxnObj trxnObj; Vector waitVector; WaitObj waitObj; int currentsize = vect.size();
						 * 
						 * for (int ii = (currentsize - 1); ii >= 0; ii--) {
						 * 
						 * trxnObj = (TrxnObj) vect.elementAt(ii); if (trxnObj.getDataName() == currentname) { if
						 * (trxnObj.getLockType() == DataObj.READ) { this.lockTable.remove(trxnObj); DataObj dataObjnew
						 * = new DataObj(trxnObj.getXId(), trxnObj.getDataName(), trxnObj.getLockType());
						 * this.lockTable.remove(dataObjnew); } }
						 * 
						 * } }
						 */
						...
						System.out.println("Converting READ lock to WRITE for data" + dataObj.getXId());

						return false;
					}
					// for case (2), Existing WRITE lock, throw an exception
			...
			else
			{
				...
				else if (dataObj.getLockType() == DataObj.WRITE)
				{
					...
					// System.out.println("Kai is removing the LOCK if it is read");
					/*
					 * int currentid = dataObj.getXId(); String currentname2 = dataObj.getDataName();
					 */

					/*
					 * if (dataObj2.getLockType() == DataObj.READ) { // wait this dataobj to remove READ lock
					 * System.out.println("Want Write, someone has Write"); // WaitLock(dataObj2);
					 * System.out.println("Want Write, someone has READ, remove it");
					 * System.out.println(dataObj2.getLockType());
					 * 
					 * this.lockTable.remove(dataObj2); TrxnObj trxnObj = new TrxnObj(dataObj2.getXId(),
					 * dataObj2.getDataName(), dataObj2.getLockType()); this.lockTable.remove(trxnObj);
					 * 
					 * return false; }
					 */
					// TrxnObj trxnQueryObj = new TrxnObj(currentid, "", -1);
					/*
					 * synchronized (this.lockTable) { Vector vectc2 = this.lockTable.elements(trxnQueryObj);
					 * 
					 * TrxnObj trxnObj; int currentsize = vectc2.size();
					 * 
					 * for (int ii = (currentsize - 1); ii >= 0; ii--) {
					 * 
					 * trxnObj = (TrxnObj) vectc2.elementAt(ii); if (trxnObj.getDataName() == currentname2) { if
					 * (trxnObj.getLockType() == DataObj.READ) {
					 * System.out.println("Removing read from other transactions"); this.lockTable.remove(trxnObj);
					 * DataObj dataObjnew = new DataObj(trxnObj.getXId(), trxnObj.getDataName(), trxnObj.getLockType());
					 * this.lockTable.remove(dataObjnew); } }
					 * 
					 * } }
					 */
